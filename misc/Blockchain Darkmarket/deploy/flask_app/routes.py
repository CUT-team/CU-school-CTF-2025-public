...
def check_admin_auth(request):
    ...
    if (token and token == expected_admin_key) or (
        url_token and url_token == expected_admin_key
    ):
        return True, None
...
def index():
    ...
    logger.info(
        f"Index accessed by {request.remote_addr} - Using protocol: {protocol} for hostname: {hostname}"
    )
    ...
    if not user_uuid:
        ...
        if is_localhost:
            ...
            response.set_cookie(
                COOKIE_NAME, user_uuid, httponly=True, secure=False, samesite="Lax"
            )
        return response
    ...
    if user_container:
        ...
        con_host = COMMAND_CONNECT.replace("<ip>", hostname).replace(
            "<port>", str(user_container[1])
        )
...
def get_captcha():
    ...
    return jsonify({"captcha_id": captcha_id, "captcha_image": captcha_image})
    ...
    with metrics.TimingContext(metrics.CONTAINER_DEPLOYMENT_DURATION):
        try:
            ...
            if not user_uuid:
                ...
                return (
                    jsonify({"error": "Session error: please refresh the page."}),
                    400,
                )
            ...
            if check_ip_rate_limit(remote_ip):
                ...
                return (
                    jsonify(
                        {
                            "error": "You have reached your max containers for this period."
                        }
                    ),
                    429,
                )
            ...
            if ENABLE_RESOURCE_QUOTAS:
                ...
                container_memory = (
                    float(CONTAINER_MEMORY_LIMIT.rstrip("M")) / 1024.0
                )  # MB -> GB
            ...
            container_name = (
                f"{base_project_name}_session_{safe_user}_{time_stamp}_{rand_suffix}"
            )
            ...
            for attempt_i in range(PORT_ALLOCATION_MAX_ATTEMPTS):
                ...
                if not port:
                    ...
                    return (
                        jsonify({"error": "No free ports. Please try again later."}),
                        503,
                    )

                logger.info(
                    f"Trying port={port} for container name={container_name} (attempt {attempt_i+1})."
                )
                ...
                config = {
                    ...
                    "environment": {
                        "FLAG": FLAG,
                        "MNEMONIC": "tourist frame critic siren upgrade profit swamp submit goose chase film uncle",
                        "ACCOUNT1": "0x41751623c51dd2cd40582459070bd333a9574eec5e4e48fec90d141c2c6e9da9,10000000000000000000000000",
                        "ACCOUNT2": "0x9e55ca5ba72cf3840328380cbd57db565c998bc08e863a32390d1b64283eefb7,100000000000000000000",
                        "RPC_PORT": "8545",
                    },
                ...
                try:
                    ...
                    logger.info(
                        f"Container {final_container.id} fully started on port {port}."
                    )
                    break  # success
                    ...
                    if "address already in use" in str(e).lower():
                        logger.warning(
                            f"Port {port} is in use externally. Releasing & skipping it."
                        )
                        release_port(port)
                        ...
                        logger.error(
                            f"Container creation+start error (not address in use): {str(e)}"
                        )
                        release_port(port)
            ...
            if not final_container:
                logger.error(
                    "All attempts exhausted without success, container not started."
                )
                return (
                    jsonify({"error": "All attempted ports failed. Try again later."}),
                    503,
                )
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to remove container after DB error: {cleanup_e}"
                    )
                return (
                    jsonify({"error": "Internal DB error storing container info."}),
                    500,
                )
...
def stop_container():
    ...
    try:
        container_data = execute_query(
            "SELECT id, port FROM containers WHERE user_uuid = %s",
            (user_uuid,),
            fetchone=True,
        )
...
def restart_container():
    ...
    try:
        container_data = execute_query(
            "SELECT id FROM containers WHERE user_uuid = %s",
            (user_uuid,),
            fetchone=True,
        )
        if not container_data:
...
def extend_container_lifetime():
    ...
    try:
        container_data = execute_query(
            "SELECT id, expiration_time FROM containers WHERE user_uuid = %s",
            (user_uuid,),
            fetchone=True,
        )
        if not container_data:
...
def admin_status():
    ...
    try:
        ...
        if not is_authorized:
            logger.warning(
                f"Unauthorized status access attempt from {request.remote_addr}"
            )
            metrics.ERRORS_TOTAL.labels(error_type="unauthorized_access").inc()
        ...
        active_containers = execute_query(
            "SELECT COUNT(*) FROM containers", fetchone=True
        )[0]
        total_containers_created = execute_query(
            "SELECT COUNT(*) FROM ip_requests", fetchone=True
        )[0]
        ...
        total_ports = execute_query(
            "SELECT COUNT(*) FROM port_allocations", fetchone=True
        )[0]
        ...
        if ENABLE_RESOURCE_QUOTAS:
            ...
            resource_stats = {
                ...
                "last_updated": time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(usage["last_updated"])
                ),
            }
        ...
        try:
            ...
            for container in containers:
                container_id, port, start_time, exp_time, user_uuid, ip_address = (
                    container
                )
                container_detail = {
                    ...
                    "expiration_time": time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(exp_time)
                    ),
                ...
                try:
                    ...
                    container_detail["status"] = container_status.get(
                        "status", "unknown"
                    )
        ...
        response = {
            **basic_info,
            "metrics": {
                ...
                "port_usage_percent": (
                    f"{(total_ports - available_ports) / total_ports * 100:.1f}%"
                    if total_ports > 0
                    else "0%"
                ),
            },
...
def metrics_endpoint():
    ...
    try:
        ...
        if not is_authorized:
            logger.warning(
                f"Unauthorized metrics access attempt from {request.remote_addr}"
            )
            return jsonify({"error": error_message}), 403
...
def logs_endpoint():
    ...
    try:
        ...
        is_authorized = (admin_key and admin_key == expected_admin_key) or (
            auth_key and auth_key == expected_admin_key
        )

        if not is_authorized:
            logger.warning(
                f"Unauthorized logs access attempt from {request.remote_addr}"
            )
        ...
        if since:
            try:
                ...
                return (
                    jsonify(
                        {
                            "error": "Invalid 'since' parameter. Expected Unix timestamp (seconds)"
                        }
                    ),
                    400,
                )
    ...
    try:
        ...
        if service_id == "all_services":
            ...
            if output_format == "text":
                ...
                # Return as JSON
                return jsonify({"services": logs_by_service})
        else:
            ...
            if logs is None:
                return (
                    jsonify(
                        {
                            "error": f"Service '{service_id}' not found or logs unavailable"
                        }
                    ),
                    404,
                )
            ...
            if output_format == "text":
                ...
                return jsonify(
                    {"service": service_id, "logs": logs.splitlines() if logs else []}
                )
    except Exception as e:
...
def handle_all_logs(tail, since_timestamp, output_format):
    ...
    try:
        ...
        for container_data in containers:
            ...
            try:
                ...
                log_data = container.logs(
                    tail=tail, since=since_timestamp, timestamps=True
                ).decode("utf-8", errors="replace")

                user_container_logs[container_id] = (
                    log_data.splitlines() if output_format == "json" else log_data
                )
            except docker.errors.NotFound:
        ...
        if output_format == "text":
            ...
            # Return as JSON
            return jsonify(
                {"services": service_logs, "containers": user_container_logs}
            )
    except Exception as e:
        ...
        if container_id:
            ...
            if not container_data:
                return (
                    jsonify(
                        {
                            "error": f"Container {container_id} not found or not managed by this deployer"
                        }
                    ),
                    404,
                )
            ...
            try:
                ...
                if output_format == "text":
                    ...
                    return jsonify(
                        {"container_id": container_id, "logs": log_data.splitlines()}
                    )
            except docker.errors.NotFound:
                return (
                    jsonify({"error": f"Container {container_id} not found in Docker"}),
                    404,
                )
            except Exception as e:
                logger.error(
                    f"Error getting logs for container {container_id}: {str(e)}"
                )
            ...
            try:
                ...
                if not containers:
                    return jsonify(
                        {"message": "No active containers found", "containers": {}}
                    )
                ...
                for container_data in containers:
                    ...
                    try:
                        ...
                        logs_by_container[container_id] = (
                            log_data.splitlines()
                ...
                if output_format == "text":
                    ...
                    return jsonify({"containers": logs_by_container})
